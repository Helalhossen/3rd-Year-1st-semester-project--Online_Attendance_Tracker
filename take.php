...
<?php
  /*
  login -> Teacher er identity thik kora hoy ekane
  addClass -> notun class add kora hoy
  */
  ...
  // ekane database connect kora hoy
  $classNode = new Node;
  ...
  background-color: lightblue;
    ...
    <?php 
     ...
     $s=$ss+1;
      ...
      echo '<h3>Class : '.$node->getCode(). ' ('.$node->getSection().') '.$node->getYear().'<strong>-</strong> '.$s.'</h3>';
    ...
    <?php
      foreach($records as $roll => $data) {
        echo '<div class="student-record">
          <span class="roll"><a href="student_profile.php?roll='.str_replace("/","-",$roll).'&code='.$node->getCode().'&year='.$node->getYear().'&section='.$node->getSection().'">'.$roll.'</a></span>: 

...
<table class="table table-bordered table-striped"> <thead> <tr> <th><img src="lg.jpg.jpg" width="1150" height="230"</th> </tr> </tbody> </table> <html> <head> <style> body { background-color: lightblue; } </style> </head>
      ...
      if(!$classes) {
        ...
        foreach($classes as $class_id) {
          ...
          echo '<div class="class"> 
            ...
            <div><strong>Session</strong> : <span class="year">'.$year.'<strong>-</strong> '.$rr.' </span></div> 
...
//<?php foreach(range(date('Y',time()),2015) as $r)
//echo '<option>'.$r.'</option>'; ?>
              </select>
              <input class="form-control" name="code" placeholder="Code , Eg : CSE-1108">
              <select class="form-control" name="section">
              <option value="-1">Choose Section</option>
              <?php foreach(range(1,1) as $r) echo '<option>'.$r.'</option>'; ?>
              </select>
              <select class="form-control" name="semester">
              <option value="-1">Choose Semester</option>
              <?php foreach(range(1,8) as $r) echo '<option>'.$r.'</option>'; ?>
              </select>
              <input class="form-control" name="start" placeholder="Starting Roll Number (Eg. 11708020)">
              <input class="form-control" name="end" placeholder="Ending Roll Number (Eg. 11708020)">
